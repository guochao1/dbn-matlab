...
maxepoch = finetuning_maxepoch;
% maxepoch=200;
fprintf(1,'\nTraining discriminative model on data by minimizing cross entropy error. \n');
...
w_class = 0.1*randn(size(w3,2)+1,2);
...
%         max_iter=3;
        max_iter = conjugate_gradient_max_iter;
...
%%
%The label for figure
% pretrain_label = concat('Pretrain Epochs:',int2str(pretrain_maxepoch));
% layers = concat('[',int2str(numhid),', ',int2str(numpen),', ',int2str(numpen2),']');
% train_label = concat('Finetuning Epochs:',int2str(maxepoch));
% figure_name = concat(pretrain_label,', ',layers,', ',train_label);
...
xlabel('Epochs (finetuning)');
%%
print('-dtiff','error.tif','-f1');

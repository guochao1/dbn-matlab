...
run_other_settings='13 rings vs the rest';
...
single_run_data_array=[runs,max(tr.epoch),tr.best_epoch,min(tr.perf),min(tr.vperf),min(tr.tperf),run_stop_cause,classified_RATIO,precision,recall,TP,(TP+FP),(TP+FN),classified_OK];
all_runs_data_array(loop_index,1:14)=[runs,max(tr.epoch),tr.best_epoch,min(tr.perf),min(tr.vperf),min(tr.tperf),run_stop_cause,classified_RATIO,precision,recall,TP,(TP+FP),(TP+FN),classified_OK];

%% echo the results of the run
...
plot(all_runs_data_array(:,1),all_runs_data_array(:,9),'r.',...
        'MarkerSize', 12)
  %precision (TEST)   
plot(all_runs_data_array(:,1),all_runs_data_array(:,10),'g','LineWidth', 1);
plot(all_runs_data_array(:,1),all_runs_data_array(:,10),'g.',...
        'MarkerSize', 8)
   
    
run_info_label2= ['all runs "max(classified-RATIO)": ' num2str(max(all_runs_data_array(:,8))) char(10)];
run_info_label2= [run_info_label2 'all runs "mean(classified-RATIO)": ' num2str(mean(all_runs_data_array(:,8))) char(10)];
run_info_label2= [run_info_label2 'all runs "max(precision)": ' num2str(max(all_runs_data_array(:,9))) char(10)];
run_info_label2= [run_info_label2 'all runs "mean(precision)": ' num2str(mean(all_runs_data_array(:,9))) char(10)];
run_info_label2= [run_info_label2 'all runs "max(recall)": ' num2str(max(all_runs_data_array(:,10))) char(10)];
run_info_label2= [run_info_label2 'all runs "mean(recall)": ' num2str(mean(all_runs_data_array(:,10))) char(10)];

run_info_label3= run_info_label2;


legend('classified RATIO ','','precision','','recall','','Location','SO');
 xlabel('ÅðáíÜëçøç(Run)', ... 
     'FontSize', 15);
 ylabel('Eðéôõ÷ßá ôçò êáôçãïñéïðïßçóçò', ...
     'FontSize', 15);
 
 title(['ÅÊÐÁÉÄÅÕÓÇ MLP ÓÅ "BACK PROPAGATION"' char(10) 'ÅÐÉÔÕ×ÇÌÅÍÅÓ ÊÁÔÇÃÏÑÉÏÐÏÉÇÓÅÉÓ (% åðß ôïõ óõíüëïõ åëÝã÷ïõ)' char(10) run_info_label char(10)] , ...
     'FontSize', 10) ;
...
plot(all_runs_data_array(:,1),all_runs_data_array(:,5),'g.',...
    'MarkerSize', 15);
%min(tr.tperf)
plot(all_runs_data_array(:,1),all_runs_data_array(:,6),'r','LineWidth', 1);
...
%TP (true positive)
% plot(all_runs_data_array(:,1),all_runs_data_array(:,11),'b','LineWidth', 2);
plot(all_runs_data_array(:,1),all_runs_data_array(:,11),'b.',...
    'MarkerSize', 25);
%TP+FP (predicted as 1)
% plot(all_runs_data_array(:,1),all_runs_data_array(:,12),'g','LineWidth', 2);
plot(all_runs_data_array(:,1),all_runs_data_array(:,12),'g.',...
    'MarkerSize', 20);
%TP+FN (truly 1)
% plot(all_runs_data_array(:,1),all_runs_data_array(:,13),'r','LineWidth', 2);
plot(all_runs_data_array(:,1),all_runs_data_array(:,13),'r.',...
    'MarkerSize', 15);
%%TP+TN  equals "classified_OK" equals "sum(decision==ttest)";
% plot(all_runs_data_array(:,1),all_runs_data_array(:,14),'m','LineWidth', 2);
plot(all_runs_data_array(:,1),all_runs_data_array(:,14),'m.',...
    'MarkerSize', 10);


legend('TP (predicted as 1 and are truly 1)','TP+FP (predicted as 1)','TP+FN (truly 1)','TP+TN="classified-OK"','Location','SO');
 xlabel('ÅðáíÜëçøç(Run)', ... 
     'FontSize', 15);
 ylabel('ÊÁÔÇÃÏÑÉÏÐÏÉÇÓÅÉÓ', ...
     'FontSize', 15);
 title(['ÅÊÐÁÉÄÅÕÓÇ MLP ÓÅ "BACK PROPAGATION" - ÐÉÍÁÊÁÓ ÓÕÃ×ÕÓÇÓ' char(10) run_info_label ' - ' 'all runs best "min(tr.tperf)": ' num2str(min(all_runs_data_array(:,6)))] , ...
     'FontSize', 10) ;
 %% PLOT THE STOP CAUSE
figure(3);
...
if (save_experiment=='Y' || save_experiment=='y');
 ...
 %Save a text file summary
  ...
  %Save the Epochs plot
    print('-dpng', '-f5', [this_experiment_path '\' filename_prefix '_conf.png']);
